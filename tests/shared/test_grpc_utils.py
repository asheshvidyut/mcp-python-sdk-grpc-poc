...
def test_get_metadata_value_empty_metadata():
    ...
    metadata: tuple[tuple[str, str | bytes], ...] = tuple()
...
def test_get_metadata_value_none_metadata():
    ...
    assert grpc_utils.get_metadata_value(None, "key1") is None  # type: ignore[arg-type]

@pytest.mark.asyncio
async def test_get_protocol_version_from_context_no_version(mock_context: AsyncMock):
...
@pytest.mark.asyncio
async def test_get_protocol_version_from_context_unsupported_version(mock_context: AsyncMock):
...
@pytest.mark.asyncio
async def test_get_protocol_version_from_context_supported_version(mock_context: AsyncMock):
    ...
    result = await grpc_utils.get_protocol_version_from_context(cast(grpc.aio.ServicerContext[Any, Any], mock_context), supported_versions)
